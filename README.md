...
- [x]When a `Backer` instance is initialized, it should be initialized with a name.

- [x]When a `Project` instance is initialized, it should be initialized with a title.

- [x]When a `ProjectBacker` instance is initialized, it should be initialized with a `Project` instance and a
`Backer` instance.

- [x]The `ProjectBacker` class is maintaining the relationship. It should have an `@@all` class
variable. When an instance is initialized, it should be
stored in this variable.

- [x]The `ProjectBacker` class should also have a class
method `.all` that returns the `@@all` class variable.

- [x]Once both classes have their attributes and readers set up, write an instance
...
- [x]Similarly, write a method on the Project class called `add_backer` that takes
  in a Backer instance and creates a `ProjectBacker` using the Backer instance
  and the current Project instance.

- [x]With `back_project` set up, the final step for the Backer class is to build an
